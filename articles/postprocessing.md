...
Postprocessing supports multiple streams, allowing you to postprocess
data in multiple ways. For example, perhaps you’d like to compare
results for a 5mm versus 6mm spatial smoothing kernel.

Each stream also asks about which files should be entered into the
...
If you choose to add a stream, you will be guided through major decision
points.
...
entered into this postprocessing stream. Instead of writing a full
regular expression, you may supply BIDS entity-value pairs that are
converted to a regex at runtime. For example:
...
Every postprocessing stream is also defined by a [BIDS description
field](https://bids-specification.readthedocs.io/en/stable/derivatives/common-data-types.html#preprocessed-or-cleaned-data).
This field (`bids_desc`) is used to label of every postprocessed for the
stream. For example, if the input filename is
...
After the global settings, the postprocessing menus walk through a
...
enable it, and key options the setup functions will prompt for.
...
across subjects by reducing high-frequency spatial noise.

You will be asked to specify the size of the smoothing kernel in
...
only voxels of similar intensities. Technical details are
[here](https://users.fmrib.ox.ac.uk/~steve/susan/susan/node18.html#SECTION00062000000000000000)
...
When ICA‑AROMA outputs are available, you can regress out the noise
...
Temporal filtering removes low- and/or high-frequency components from
...
We support both fslmaths -bptf and a Butterworth temporal filter. The
...
between these filtering methods.

Per FSL’s documentation, -bptf uses a local fit of a straight line
...
the signal of interest, particularly for single-event experiments.

Butterworth filtering is a common temporal filtering technique used in
fMRI postprocessing to attenuate low-frequency drifts and/or
...
but have bigger risks of artifacts).

One of the virtues of FSL’s Gaussian line smoothing approach is that it
has a very gradual rolloff. That said, the frequency response is broad
and not sharply defined, which can lead to partial attenuation of
...
Here is the order of prompts with example responses. Note that if you
...
‘Scrubbing’ refers to a set of procedures for reducing the impact of
...
Temporal filtering removes specific *frequencies* from the data, so we
cannot simply delete volumes from the time series and hope to have the
filtering work as intended. Moreover, if we apply the temporal filter
prior to scrubbing, we can induce ‘ringing artifacts’ where large signal
changes due to head motion are propagated to adjacent timepoints (Carp,
2012, NeuroImage). One effective way to handle this is to interpolate
over bad volumes (cubic spline is common), then apply the temporal
filter, and later scrub out the bad (now interpolated) timepoints. This
is how
[nilearn](https://nilearn.github.io/stable/modules/generated/nilearn.signal.clean.html#nilearn.signal.clean)
and [XCP-D](https://xcp-d.readthedocs.io/en/latest/workflows.html)
approach the problem. BrainGnomes also allows for spline interpolation
as a precursor to temporal filtering.

Another scrubbing interaction occurs with confound regression. If we
...
If you enable scrubbing, you will be asked to define what constitutes a
‘scrubbed’ volume. This should be a single logical expression in R
...
In addition, your scrubbing expression can specify a range of volumes to
scrub using a format like: ‘-1:1; dvars \> 1.5’. The part before the
semicolon (-1:1) defines a temporal window around the bad timepoint. For
example, ‘-1:0’ scrubs the bad timepoint and the volume before it. If
omitted, only the bad timepoint (0) is scrubbed. The part after the
semicolon (‘dvars \> 1.5’) defines the thresholding condition.

#### Adding scrubbing spike regressors to postprocessed confounds

Next, you will be asked whether you want to add any volumes identified
as ‘bad’ by the scrubbing expression to your postprocessed confounds.tsv
file. This is only relevant if you enabled to the (preceding) confound
calculate step. In this case, BrainGnomes will create spike regressors
(1 at the bad timepoint, 0 elsewhere) for each volume flagged, adding
them to the final confounds.tsv file. Note that if you ‘apply’ scrubbing
– that is, remove the scrubbed timepoints – the spike regressors will
*not* be added to the postprocessed confounds since they would just be
all 0 anyhow.

Adding these spike regressors to the confounds.tsv file is useful if you
plan to include them as nuisance regressors in a larger (typically GLM)
analysis, rather than deleting those volumes entirely during scrubbing.

`Add any spike regressors (bad volumes) to postprocessed confounds.tsv file?`

#### Interpolation of scrubbed volumes

As noted above, temporal filtering can be problematic when there are
...
intensity artifacts (e.g., on fMRIPrep’s carpet plots).

`Interpolate scrubbed values?`

#### Removing scrubbed volumes

The term ‘scrubbing’ traditionally referred to removing bad
...
connectivity. This remains one of its most common uses.

If you would like to remove bad timepoints (based on the scrubbing
expression), say ‘yes’ to this question:

`Remove scrubbed timepoints?`

This will delete the bad volumes as well as corresponding rows in your
postprocessed confounds file (if relevant).
...
fixed target of 100, producing approximate local percent-signal-change
units. “Guarded” means reliable baselines use ordinary
`100 / local_baseline` scaling, very low positive denominators use a
fixed lower bound, and unidentified denominators use a conservative
run-level fallback. The guard protects division; it does not clip
observations or mask voxels. Both modes use the same user-specified
filename prefix. BrainGnomes estimates the multiplier from a
conservative region of stable, positive-signal functional voxels; an
external fMRIPrep or template mask is not required. This reference
region is an intensity-calibration mask, not an anatomical ROI or the
mask that determines which voxels may be analyzed. It is restricted to
voxels with adequate signal and without extreme temporal noise or
frequent intensity spikes.

In practical terms, choose `run_scalar` for conventional FSL-style
scaling and comparable target-based coefficient magnitudes. Choose
`voxel_psc` when the scientific quantity of interest is local fractional
response and coefficients should approximate percent signal change. Use
the same mode across every run and participant entering an analysis.
BrainGnomes handles the reference and low-signal safeguards
automatically.
...
For `run_scalar`, if this spatial median is $`L`$, BrainGnomes
multiplies the entire run by `target / L`. Thus, a target of 10,000
means that, immediately after scaling, the spatial median across
reference-region voxels of their 10%-trimmed temporal means is 10,000.
It does **not** mean that the temporal median, whole-brain mean, or
median of every value in the final 4D image is 10,000.

For `voxel_psc`, BrainGnomes calculates the same 10%-trimmed baseline at
every voxel and normally applies `100 / voxel_baseline`. A positive
denominator below 20% of $`L`$ is floored at that value, while an
unidentified denominator receives the conservative fallback `100 / L`.
This bounds unstable amplification without removing the voxel. Floor and
fallback voxels are retained but are not exact local PSC. The reference
region is never applied as a PSC validity mask: `apply_mask` remains the
only postprocessing step that determines which voxels are removed. No
time-series values are clipped. The exact PSC multiplier map and
category counts are saved for QA. Counts and percentages within the
conservative automask appear in the info log; complete-grid counts
appear only at debug level because they are commonly dominated by
background voxels.
...
3.  In the image produced by those spatial steps, it calculates each
    ...
    median $`L`$ described above. It immediately applies either
    `target / L` or the denominator-guarded voxelwise PSC multiplier
    map.
...
This ordering distinction does not arise for `run_scalar`: one global
constant commutes with linear spatial smoothing. For `voxel_psc`,
however, smoothing and scaling order does have a substantive meaning.
Scaling first and smoothing afterward would average neighboring voxels’
fractional time series, giving low- and high-baseline voxels similar
spatial weight after normalization. BrainGnomes instead smooths the
acquired signal first and then divides by that smoothed signal’s robust
baseline. This improves denominator stability, avoids spreading large
low-baseline multipliers into neighbors, and makes a PSC coefficient
describe the exact smoothed series used for modeling. The tradeoff is
that the result is percent change relative to a local smoothed baseline,
not PSC calculated independently at each voxel before smoothing. The
distinction is usually small in homogeneous tissue but can matter at
tissue boundaries, near dropout, or with a large smoothing kernel. Use a
separate stream with spatial smoothing disabled when unsmoothed
...
AFNI’s default numerical operation uses the ordinary temporal mean and
an upper cap on individual scaled observations. BrainGnomes PSC instead
bounds or replaces unstable denominators while retaining a time-constant
linear multiplier; it does not clip observations. `run_scalar` remains
the single-factor alternative. The shared design choice is to scale
...
motion parameters and their derivatives/squared terms.
...
During the setup of postprocessing, you will be asked whether to keep
