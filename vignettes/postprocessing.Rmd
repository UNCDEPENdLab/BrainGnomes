...
`setup_intensity_normalization()` places different runs on a common intensity
scale by multiplying every value in a run by one run-specific constant. The
user selects a `target` (default 10,000), which should be kept the same across
all runs and participants that will be compared in an analysis. BrainGnomes
...
If this spatial median is $L$, BrainGnomes multiplies the entire run by
...
3. In the image produced by those spatial steps, it calculates each reference
   ...
   above. It immediately applies `target / L` to the entire 4D run.
...
operation is different—voxelwise mean-100 scaling with an upper cap—whereas
BrainGnomes applies one uncapped robust scalar to the complete run. The shared
design choice is to scale after spatial preprocessing and before temporal
modeling.
...
See the dedicated
[`Run-wise Intensity Normalization in BrainGnomes`](intensity_normalization.html)
